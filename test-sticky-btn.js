$(document).on('productDataLoaded', function() {
    ...
    $(window).resize(function() {
        if ($(window).width() <= 768) {
            if ($(".qsb-wrapper .qsb-product__current-price br").length === 0) {
                $(".qsb-wrapper .qsb-product__current-price").append("<br>");
            }
        } else { $(".qsb-wrapper .qsb-product__current-price br").remove() }
    });
});
