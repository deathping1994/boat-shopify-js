$(document).on(`page:load`, function () {
  ...
  listItems.each(function (idx, li) {
    var x = $(li).find(".div-1");
    if ($(li).hasClass("breadcrumb__item breadcrumb__item--current")) {
      x.removeClass("grey");
      x.addClass("red");
    }
    if ($(li).hasClass("breadcrumb__item breadcrumb__item--completed")) {
      x.removeClass("grey");
      x.addClass("red");
    }
    if ($(li).hasClass("breadcrumb__item breadcrumb__item--blank")) {
      x.removeClass("red");
      x.addClass("grey");
    }
  });
  ...
  if (
    $("header nav ol li:nth-child(1)").hasClass(
      "breadcrumb__item breadcrumb__item--current"
    ) &&
    $("header nav ol li:nth-child(2)").hasClass(
  ...
  if (
    ...
    $("header nav ol li:nth-child(3)").hasClass(
      "breadcrumb__item breadcrumb__item--completed"
    )
  ) {
...
$(document).on(`page:load page:change`, function () {
  $("#checkout_reduction_code").on("input", function () {
    if (
      ...
      $(".field__input-btn").attr("style", "background :#d3d3d3 !important");
    }
  });

  $("#checkout_shipping_address_phone").removeAttr("data-phone-formatter");
  $("#checkout_shipping_address_phone").removeAttr(
    "data-phone-formatter-country-code"
  );
  $("#checkout_shipping_address_phone").removeAttr(
    "data-phone-formatter-country-select"
  );
  ...
  if ($.inArray(inputPincode, pincodes) === -1) {
    ...
    if ($(".address-fields :nth-child(17) p").length == 0) {
      $(".address-fields :nth-child(17) div").after(
        `<p class="field__message field__message--error" id="error-for-zip">Sorry, we don't deliver to this pincode.</p>`
      );
    }
        ...
        if (quan > 1) {
          for (var i = 0; i < GETcart.items.length; i++) {
            if (prod.includes(GETcart.items[i].product_id)) {
              ...
              if (flag != 0) {
                if (i == GETcart.items.length - 1) {
                  $.post("/cart/update.js", "updates[" + variant + "]=0").then(
                    () => {
                      window.location.reload();
                    }
                  );
  ...
  if ($(".reduction-code__text")[1]) {
    if ($(".reduction-code__text")[1].innerHTML == "INFO") {
      $(".field__message.field__message--error")[1].innerHTML =
        "This coupon can only be applied to one product.";
  ...
  $(".order-summary__section--discount .field__input-btn").on(
    "click touchstart",
    function () {
        ...
        if ($(".notice.notice--warning .notice__content .notice__text").text().length > 0) {
          $.ajax((type = "POST"),(url = "https://boat-bulk.farziengineer.co/couponlog"),
            (data = {
              coupon: basecode,
              log: $(
                ".notice.notice--warning .notice__content .notice__text"
              ).text(),
            }),(headers = {
              "Content-Type": "application/json",
            })
          );
          console.log("sent if 3");
          clearInterval(v);
        }
      }, 1000);
    }
