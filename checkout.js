$(document).on(`page:load`, function() {
    ...
    $(document).on(`page:load page:change`, function() {
      ...
      // fetch('https://boatlifestylein.myshopify.com/api/2021-01/graphql.json', {
      //   method: 'POST',
      //   headers: {
      //     'Content-Type': 'application/json',
      //     'Accept': 'application/json',
      //     'X-Shopify-Storefront-Access-Token': '99bc9b73d8706beadf011a99be8653ee',
      //   },
      //   body: JSON.stringify({
      //     query:`query {
      //         products(first:2) {
      ...
      fetch('https://boatlifestylein.myshopify.com/api/2021-01/graphql.json', {
                    ...
                    body: JSON.stringify({
                      mutation: `mutation {
                        checkoutLineItemsReplace(lineItems: [{ variantId: "MzIwNjAyMjc4NDYyNDI=", quantity: 1 }], checkoutId: "NDhjZWMzYzFkZTA4NGVjNWNkMjczZjNjMzkyYTlmNzM=",
                        ) {
                          checkout {
                             id
                             lineItems(first:1) {
        ...
        if(!inputPincode){
            ...
            // $.ajax({
            //   url: 'https://boatlifestylein.myshopify.com/cart.js',
            //   type: "GET",
            //   dataType: "json",
      ...
      $('.field__input-btn.btn').on("click", function (event) {
        if ($('#checkout_reduction_code')[0].value == "MaskOn") {
          ...
          if ($('.reduction-code__text')[0]) {
            ...
            $.get('/cart.js').then(response => {
              ...
              for (var i = 0; i < GETcart.item_count; i++) {
                if (GETcart.items[i].id == 32060227846242) {
                  var quan = GETcart.items[i].quantity;
                  $.post('/cart/update.js', {
                    updates: {
                      32060227846242: quan + 1,
                    }
                  }).then(response => {
      ...
      $('.icon-svg.icon-svg--color-adaptive-dark.icon-svg--size-12.icon-svg--block').on("click", function () {
        $.get('/cart.js').then(response => {
          var GETdetails = response;
          GETdetails = $.parseJSON(GETdetails)
          for (var j = 0; j < GETdetails.item_count; j++) {
            if (GETdetails.items[j].id == 32060227846242) {
              var count = GETdetails.items[j].quantity;
              $.post('/cart/update.js', {
                updates: {
                  32060227846242: count - 1
                }
              }).then(response => {
                window.location.reload();
              })
